...
<style>
    ...
    /* تنسيقات العناوين الاحترافية الجديدة */
    .premium-header {
...
# ==========================================
# 3. إدارة حالة البيانات الافتراضية والترقية
# ==========================================
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
        ...
        if os.path.exists(indiv_folder):
            ...
            for p in kd["partitioners"]:
                ...
                if p.get("total_f", 0) == 0 and p.get("total_k", 0) == 0 and p.get("total_s", 0) == 0:
                    continue
...
<div style='text-align: center; padding-bottom: 20px; border-bottom: 1px solid rgba(255,255,255,0.1); margin-bottom: 20px;'>
    ...
    <span style='font-size: 16px; font-weight: 600; color: #d4af37;'> Legal Contracts</span>
</div>
""", unsafe_allow_html=True)

menu = ["📝 منظومة عقود البيع", "🤝 منظومة القسمة الرضائية", "🧮 حاسبة الأراضي", "📂 أرشيف العقود", "🖨️ إدارة المستندات (فردي)", "🔄 الاسترجاع من ملف (Backup)"]
...
if choice == "🔄 الاسترجاع من ملف (Backup)":
    ...
    if uploaded_file is not None:
        try:
            ...
            doc_type = loaded_data.get("doc_type")
            if not doc_type:
                if "partitioners" in loaded_data or "moraث" in loaded_data: doc_type = "kesma"
                else: doc_type = "sale"
            ...
            st.error(f"❌ حدث خطأ في قراءة الملف: تأكد أنه ملف backup_data.json سليم.")

# ==========================================
# 7. واجهة: حاسبة الأراضي 
# ==========================================
...
# ==========================================
# 8. واجهة: منظومة عقود البيع
# ==========================================
elif choice == "📝 منظومة عقود البيع":
    if st.session_state.current_archive_id is not None and st.session_state.get('loaded_doc_type') != 'sale':
        st.session_state.current_archive_id = None
    ...
    with col1:
        ...
        if fd["is_heirs_s"]: 
            ...
            with wc3: 
                s_m_date = parse_date_safe(fd.get("s_morath_date"))
                new_s_m_date = st.date_input("تاريخ جلسة الحكم", value=s_m_date, key="s_m_date")
                fd["s_morath_date"] = new_s_m_date.isoformat()
        ...
        for i, s in enumerate(fd["sellers"]):
            with st.expander(f"👤 بيانات البائع رقم {i+1}: {s.get('name','')}", expanded=True):
                ...
                c3, c4 = st.columns(2)
                unique_age_label = "السن" + " " * (i + 1)
                with c3: s["age"] = st.text_input(unique_age_label, value=calc_s_age if calc_s_age else s.get("age", ""))
                with c4: 
                    s_id_date_val = parse_date_safe(s.get("id_date"))
                    s["id_date"] = st.date_input("تاريخ إصدار البطاقة", value=s_id_date_val, key=f"s_date_{i}").isoformat()
                s["address"] = st.text_input(f"العنوان", value=s.get("address", ""), key=f"s_addr_{i}")
                
                if len(fd["sellers"]) > 1:
                    st.markdown("<div class='delete-btn'>", unsafe_allow_html=True)
                    if st.button(f"🗑️ حذف البائع {i+1}", key=f"del_s_{i}"):
                        fd["sellers"].pop(i); st.rerun()
                    st.markdown("</div>", unsafe_allow_html=True)

        if st.button("➕ أضف بائع آخر"):
            fd["sellers"].append({"name": "", "id": "", "address": "", "id_date": today_iso, "job": "", "age": ""}); st.rerun()
    ...
    with col2:
        ...
        if fd["is_heirs_b"]: 
            ...
            with wc3: 
                b_m_date = parse_date_safe(fd.get("b_morath_date"))
                new_b_m_date = st.date_input("تاريخ جلسة الحكم", value=b_m_date, key="b_m_date")
                fd["b_morath_date"] = new_b_m_date.isoformat()
        ...
        for i, b in enumerate(fd["buyers"]):
            with st.expander(f"👤 بيانات المشتري رقم {i+1}: {b.get('name','')}", expanded=True):
                ...
                c7, c8 = st.columns(2)
                unique_age_label_b = "السن " + " " * (i + 1)
                with c7: b["age"] = st.text_input(unique_age_label_b, value=calc_b_age if calc_b_age else b.get("age", ""))
                with c8: 
                    b_id_date_val = parse_date_safe(b.get("id_date"))
                    b["id_date"] = st.date_input("تاريخ الإصدار ", value=b_id_date_val, key=f"b_date_{i}").isoformat()
                b["address"] = st.text_input(f"العنوان ", value=b.get("address", ""), key=f"b_addr_{i}")
                
                if len(fd["buyers"]) > 1:
                    st.markdown("<div class='delete-btn'>", unsafe_allow_html=True)
                    if st.button(f"🗑️ حذف المشتري {i+1}", key=f"del_b_{i}"):
                        fd["buyers"].pop(i); st.rerun()
                    st.markdown("</div>", unsafe_allow_html=True)

        if st.button("➕ أضف مشتري آخر"):
            fd["buyers"].append({"name": "", "id": "", "address": "", "id_date": today_iso, "job": "", "age": ""}); st.rerun()
    ...
    for i, l in enumerate(fd["lands"]):
        ...
        with col_del:
            if len(fd["lands"]) > 1:
                st.markdown("<div class='delete-btn'>", unsafe_allow_html=True)
                if st.button("❌ حذف القطعة", key=f"del_sell_land_{i}"):
                    fd["lands"].pop(i); st.rerun()
                st.markdown("</div>", unsafe_allow_html=True)
    ...
    with tc3: 
        c_date_val = parse_date_safe(fd.get("c_date"))
        new_c_date = st.date_input("تاريخ العقد", value=c_date_val, key="c_date_picker")
        fd["c_date"] = new_c_date.isoformat()
    with tc4:
        t_date_val = parse_date_safe(fd.get("t_date"))
        new_t_date = st.date_input("تاريخ اليوم", value=t_date_val, key="t_date_picker")
        fd["t_date"] = new_t_date.isoformat()
    ...
    if 'zip_data' in st.session_state:
        zip_name_sale = f"عقد_بيع_{prev_buyer}_مشتراه_من_{prev_seller}.zip"
        st.download_button("📥 تحميل جميع مستندات البيع (ملف ZIP)", data=st.session_state.zip_data, file_name=zip_name_sale, mime="application/zip", type="secondary", use_container_width=True)

# ==========================================
# 9. واجهة: منظومة القسمة الرضائية
# ==========================================
elif choice == "🤝 منظومة القسمة الرضائية":
    if st.session_state.current_archive_id is not None and st.session_state.get('loaded_doc_type') != 'kesma':
        st.session_state.current_archive_id = None
    ...
    with c3: 
        c_date_val = parse_date_safe(kd.get("c_date"))
        new_c_date = st.date_input("تاريخ شرط القسمة", value=c_date_val, key="k_date_picker")
        kd["c_date"] = new_c_date.isoformat()
    ...
    with w3: 
        m_date = parse_date_safe(kd.get("morath_date"))
        new_m_date = st.date_input("تاريخ جلسة الوراثة", value=m_date, key="m_date_picker")
        kd["morath_date"] = new_m_date.isoformat()
    ...
    for ml_idx, ml in enumerate(kd["main_lands"]):
        ...
        with col_del:
            if len(kd["main_lands"]) > 1:
                st.markdown("<div class='delete-btn'>", unsafe_allow_html=True)
                if st.button("❌ حذف القطعة", key=f"del_main_land_{ml_idx}"):
                    kd["main_lands"].pop(ml_idx); st.rerun()
                st.markdown("</div>", unsafe_allow_html=True)
    ...
    for p_idx, p in enumerate(kd["partitioners"]):
        with st.expander(f"👤 المتقاسم رقم {p_idx+1}: {p.get('name', '') if p.get('name', '') else '(بدون اسم)'}", expanded=True):
            col_p_title, col_p_del = st.columns([5, 1])
            with col_p_del:
                if len(kd["partitioners"]) > 1:
                    st.markdown("<div class='delete-btn'>", unsafe_allow_html=True)
                    if st.button("❌ حذف الوريث", key=f"del_part_btn_{p_idx}"):
                        kd["partitioners"].pop(p_idx); st.rerun()
                    st.markdown("</div>", unsafe_allow_html=True)
            ...
            unique_age_label = "السن" + " " * (p_idx + 1)
            with p_c4: p["age"] = st.text_input(unique_age_label, value=calc_age if calc_age else p.get("age",""))
            with p_c5: 
                p_date_val = parse_date_safe(p.get("nat_id_date"))
                new_p_date = st.date_input("تاريخ البطاقة", value=p_date_val, key=f"pk_date_{p_idx}")
                p["nat_id_date"] = new_p_date.isoformat()
            ...
            for l_idx, l in enumerate(p["lands"]):
                ...
                with col_del:
                    if len(p["lands"]) > 1:
                        st.markdown("<div class='delete-btn'>", unsafe_allow_html=True)
                        if st.button("❌ حذف القطعة", key=f"del_part_land_{p_idx}_{l_idx}"):
                            p["lands"].pop(l_idx); st.rerun()
                        st.markdown("</div>", unsafe_allow_html=True)
...
# ==========================================
    ...
    if search_query:
        df = df[(df['seller_name'].str.contains(search_query, na=False)) | (df['buyer_name'].str.contains(search_query, na=False))]
            ...
            with st.expander(f"📄 {row['seller_name']} | بتاريخ: {format_custom_date(row['contract_date'])}"):
                ...
                with col_btn_edit:
                    st.button(
                        f"✏️ استرجاع وتعديل", 
                        key=f"edit_{row['id']}",
                        on_click=load_from_archive,
                        args=(row['id'], row['raw_data']),
                        use_container_width=True
                    )
                with col_btn_del:
                    if st.button(f"🗑️ حذف نهائي", key=f"del_arc_{row['id']}", type="primary", use_container_width=True):
                        delete_from_db(row['id'])
                        st.rerun()

# ==========================================
# 11. واجهة: إدارة المستندات (الفردية للبيع)
# ==========================================
elif choice == "🖨️ إدارة المستندات (فردي)":
    st.title("🖨️ طباعة واستخراج المستندات الفردية")
    st.info("💡 ملاحظة: ملفات القسمة يتم استخراجها آلياً دفعة واحدة من صفحة القسمة نظراً لارتباطها بعدة ورثة. هذه الصفحة مخصصة لاستخراج ملف واحد من قوالب (البيع).")
    ...
    if os.path.exists(sale_folder):
        ...
        if files:
            ...
            if 'ready_file_data' in st.session_state and st.session_state.get('ready_file_name') == selected_file:
                ...
                # استخراج اسم الملف الأصلي ودمجه بالتنسيق الجديد
