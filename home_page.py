...
def show_page():
    ...
    # ---- بناء الأعمدة البيانية (تعديل لإخفاء الأصفار بشكل أنيق) ----
    bars_html = ""
    for i in range(len(monthly_counts)):
        cnt = monthly_counts[i]
        cls = bar_classes[i]
        lbl = monthly_labels[i]
        short = MONTHS_SHORT[lbl[1]][:3]
        delay = 0.5 + i * 0.05
        
        # حساب الطول والشفافية
        h = int((cnt / max_c) * 64) if max_c > 0 else 0
        h = max(3, h) # الخط الأدنى حتى لا يختفي تماماً
        op = "0.2" if cnt == 0 else "1"
        
        bars_html = (bars_html
            + '<div class="bc_h">'
            + '<div class="bfill_h ' + cls + '" style="height:' + str(h) + 'px; animation-delay:' + str(delay) + 's; opacity:' + op + ';"></div>'
            + '<div class="bml_h">' + short + '</div>'
            + '</div>')

    # ---- بناء آخر المعاملات (تعديل ألوان العلامات - Badges) ----
    recent_html = ""
    icons_list = ["📝", "🤝", "📋"]
...
.nav-grid_h{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:18px}

/* الكروت المحسنة */
.ncard_h{background:#fff;border-radius:16px;border:1.5px solid #ede7de;padding:20px 10px;cursor:default;text-align:center;transition:all 0.3s cubic-bezier(0.22,1,0.36,1);position:relative;overflow:hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.02);}
.ncard_h::before{content:'';position:absolute;top:0;left:0;right:0;height:4px;transform:scaleX(0);transform-origin:right;transition:transform 0.3s ease}
...
.ncard_h:hover{border-color:#c9a84c;transform:translateY(-6px);box-shadow:0 12px 24px rgba(201,168,76,0.15);}

/* الأيقونات المكبرة */
.nc-icon_h{width:56px;height:56px;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:28px;margin:0 auto 12px;}
...
.nc-title_h{font-size:15px;font-weight:800;color:#1e3d2f;line-height:1.3}
.nc-sub_h{font-size:11px;color:#a89880;margin-top:4px}

.lower_h{display:grid;grid-template-columns:5fr 4fr;gap:12px;margin-bottom:14px}
.chart-box_h{background:#fff;border-radius:14px;border:1.5px solid #ede7de;padding:16px; box-shadow: 0 4px 6px rgba(0,0,0,0.02);}
.chart-head_h{display:flex;justify-content:space-between;align-items:center;margin-bottom:12px}
.ch-title_h{font-size:12px;font-weight:800;color:#6b5e50;letter-spacing:0.5px}
.ch-leg{display:flex;gap:8px}
.ld{display:flex;align-items:center;gap:4px;font-size:10px;color:#a89880;font-weight:600;}
.ld span{width:8px;height:8px;border-radius:2px}
...
.bml_h{font-size:9px;color:#b5a898;font-weight:bold;}

.recent-box_h{background:#fff;border-radius:14px;border:1.5px solid #ede7de;padding:16px; box-shadow: 0 4px 6px rgba(0,0,0,0.02);}
.rb-title_h{font-size:12px;font-weight:800;color:#6b5e50;letter-spacing:0.5px;margin-bottom:12px}
.ritem_h{display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid #f5f0ea}
.ritem_h:last-child{border-bottom:none;padding-bottom:0}
.ri-dot_h{width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:16px;flex-shrink:0}
.ri-info_h{flex:1;min-width:0}
.ri-name_h{font-size:12px;font-weight:800;color:#2c2416;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.ri-time_h{font-size:10px;color:#b5a898;margin-top:2px}

/* الشارات الملونة المحسنة */
.ri-tag_h{font-size:10px;padding:3px 8px;border-radius:6px;font-weight:800;flex-shrink:0;border: 1px solid transparent;}
.tg{background:#d1e7dd;color:#0f5132;border-color:#badbcc;}
.tb{background:#fff3cd;color:#856404;border-color:#ffeeba;}
.tw{background:#cff4fc;color:#055160;border-color:#b6effb;}

.prog-sect{background:#fff;border-radius:14px;border:1.5px solid #ede7de;padding:16px;margin-bottom:14px; box-shadow: 0 4px 6px rgba(0,0,0,0.02);}
.prog-title_h{font-size:12px;font-weight:800;color:#6b5e50;letter-spacing:0.5px;margin-bottom:14px}
.prog-row_h{display:flex;align-items:center;gap:12px;margin-bottom:12px}
.prog-row_h:last-child{margin-bottom:0}
.prog-icon_h{font-size:16px;width:28px;text-align:center;flex-shrink:0}
.prog-label_h{font-size:12px;font-weight:700;color:#4a3d30;width:90px;flex-shrink:0}
.prog-bar-bg_h{flex:1;height:10px;background:#f0ebe4;border-radius:5px;overflow:hidden}
.prog-bar-fill_h{height:100%;border-radius:5px;animation:slideRight_h 1s cubic-bezier(0.22,1,0.36,1) both}
.pf-green{background:#2d5a4e;animation-delay:0.5s}
.pf-amber{background:#c9a84c;animation-delay:0.6s}
.prog-val_h{font-size:12px;font-weight:800;color:#6b5e50;width:36px;text-align:left;direction:ltr;flex-shrink:0}
</style>
